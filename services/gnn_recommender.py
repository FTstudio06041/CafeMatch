...
#
# 為什麼是 5 而不是 50：
#   quiz_projector 把五維投影到 384 維時會把差異壓掉 —— 輸入向量彼此 cosine 只有
#   0.61，投影後卻變成 0.97，五種需求全擠進同一個窄錐（BERT 句向量本來就有
#   anisotropy，2286 個既有使用者彼此的 cosine 平均也高達 0.53）。
#   在這種空間裡取 top-50 鄰居，等於對前 2.2% 的人做平均，測驗訊號被洗掉。
#
#   實測不同需求的前十名兩兩重疊：k=50 是 8.0/10（幾乎分不出來），
#   k=10 是 7.3，k=5 降到 5.1。再往下 k=3 雖然重疊剩 4.3，
#   但抗噪性從 9.2/10 掉到 7.6 —— 使用者多講一項偏好就會換掉快三家，太跳。
#   k=5 在「分得出需求」與「不會亂跳」之間最平衡。
#
#   （試過先減掉均值再算 cosine 來修 anisotropy，對鄰居區分度沒有幫助，
#     k=50 時反而更差，所以沒有採用。）
NEW_USER_TOP_K = 5
