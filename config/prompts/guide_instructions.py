# 情境配對引導
QUIZ_CONSENT_INSTRUCTION = "【任務】使用者已同意進行情境配對。請在您的回覆中，確切包含字串：「[SHOW_QUIZ_CARD]」，並可附帶一句簡短自然的引導（例如：太好了！那請點擊下方的情境配對小卡，我們馬上開始～）。絕不要推薦店家，也不要再問其他問題。"
QUIZ_INVITATION_INSTRUCTION = "【任務】請詢問使用者：「為了給您更精準的推薦，願意先花 30 秒用『情境配對小卡』快速勾選一下這次的需求（時間、氛圍、預算等）嗎？」請用語氣自然的方式發問，並在句子中自然帶到「配對」一詞。"
...
# 偏好萃取
PREFERENCE_EXTRACTION_TASK = "你是一個精準的語意分析工具。你的任務是從以下對話紀錄中，萃取出使用者的咖啡廳偏好維度（如 purpose, vibe, taste, budget, special 等），並判斷使用者是否同意進行情境配對。"
